...
-- address — fine for new wallets, expensive for high-activity ones.
...
CREATE INDEX IF NOT EXISTS idx_bpos_stakes_producer ON bpos_stakes(producer_key);
CREATE INDEX IF NOT EXISTS idx_bpos_stakes_address ON bpos_stakes(stake_address);
...
CREATE TABLE IF NOT EXISTS cr_election_tallies (
    term            INT         NOT NULL,
    candidate_cid   VARCHAR(34) NOT NULL,
    nickname        VARCHAR(128) NOT NULL DEFAULT '',
    final_votes_sela BIGINT     NOT NULL DEFAULT 0,
    ...
    PRIMARY KEY (term, candidate_cid)
);
CREATE INDEX IF NOT EXISTS idx_cr_election_term ON cr_election_tallies (term, final_votes_sela DESC);
-- candidate_cid lookup is hit on every /cr/members/{cid}/profile call
-- to find every term this candidate participated in. The existing
-- (term, final_votes_sela DESC) index doesn't help — its leading
-- column is `term`, not candidate_cid. Without this, the multi-term
-- TermPills row on the candidate page costs a full table scan.
CREATE INDEX IF NOT EXISTS idx_cr_election_candidate ON cr_election_tallies (candidate_cid, term);
...
CREATE INDEX IF NOT EXISTS idx_proposals_did ON cr_proposals (cr_member_did);
CREATE INDEX IF NOT EXISTS idx_proposals_recipient ON cr_proposals (recipient);
...
CREATE INDEX IF NOT EXISTS idx_cr_reviews_did ON cr_proposal_reviews (did);
...
-- ============================================================
-- Grant read-only access to the API user
-- ela_api needs SELECT on all tables; ela_indexer owns them.
-- ============================================================
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'ela_api') THEN
    EXECUTE 'GRANT SELECT ON ALL TABLES IN SCHEMA public TO ela_api';
    EXECUTE 'ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO ela_api';
  END IF;
END $$;
